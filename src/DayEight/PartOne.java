...
public class PartOne {
    public int run(String file, int iterations) {
        ...
        if (iterations > checking.size()) {
            return -1;
        }

        int index = 0;
        while (index < iterations) {
            Map.Entry<Long, Vector<Point>> value = checking.get(index);
            ...
            if (firstCircuit == null && secondCircuit == null) {
                ...
                // They're already in a circuit do nothing.
        ...
        circuits.sort(Comparator.comparingInt(c -> -c.size()));
